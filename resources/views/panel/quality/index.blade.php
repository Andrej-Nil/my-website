        ...
        <div data-group data-sortable-list data-api="{{route('quality.update.sort')}}" class="list sortable-list">
            ...
            @forelse($qualityList['data'] as $quality)
                <div draggable="true" data-sortable-item="{{$quality['id']}}" class="list-item">
                    ...
                    <a href="{{route('panel.qualities.edit', $quality['id'])}}" class="list-item__btn" title="Редоктировать">
                        <img src="{{asset('panel-assets/img/icons/edit-icon.svg')}}" class="list-item__icon" alt="">
                    </a>
                    <form action="{{route('panel.qualities.delete', $quality['id'])}}" method="post" title="Удалить">
                        @csrf
                        @method('DELETE')
